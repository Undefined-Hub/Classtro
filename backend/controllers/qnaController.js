...
const editQuestion = async (req, res) => {
  try {
    ...
    if (String(q.authorId) !== String(userId))
    ...
    res
      .status(500)
      .json({ message: "Failed to edit question", error: err.message });
  }
};
...
const deleteQuestion = async (req, res) => {
  try {
    ...
    if (!isAuthor && !isTeacher)
      return res.status(403).json({ message: "Not allowed" });
    if (q.isAnswered && isAuthor)
      return res
        .status(400)
        .json({ message: "Cannot delete answered question" });
...
const upvoteQuestion = async (req, res) => {
  try {
    ...
    if (userRole === "TEACHER" || userRole === "ADMIN") {
      return res
        .status(403)
        .json({ message: "Teachers cannot upvote questions" });
    }
    ...
    try {
      ...
      const session = await Session.findById(q.sessionId).lean();
      emitToSession(session.code, "qna:question:upvoted", {
        questionId: id,
        delta: 1,
      });
    ...
    res
      .status(500)
      .json({ message: "Failed to upvote question", error: err.message });
  }
};
...
const markAnswered = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to mark answered", error: err.message });
  }
};
