  ...
  try {
    ...
    const feedbackData = {
      ...
      metadata: {
        ...
        timestamp:
          validatedData.metadata?.timestamp || new Date().toISOString(),
        console: Array.isArray(validatedData.metadata?.console)
          ? validatedData.metadata.console.slice(0, 10)
          : [],
      },
      status: "new",
...
const getFeedbackStats = async (req, res) => {
  try {
    ...
    const criticalBugs = await Feedback.countDocuments({
      type: "bug",
      severity: "critical",
    });
