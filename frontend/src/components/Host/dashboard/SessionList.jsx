...
const SessionList = ({ sessions, loading, error, onSessionClick, onCreateSession }) => (
  <div>
    ...
    {loading ? (
      <div className="text-center py-8 text-blue-600 dark:text-blue-400">Loading sessions...</div>
    ) : error ? (
      <div className="text-center py-8 text-red-600 dark:text-red-400">{error}</div>
    ) : sessions.length > 0 ? (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sessions.map(session => (
          <SessionCard key={session._id} session={session} onSessionClick={onSessionClick} />
        ))}
      </div>
    ) : (
