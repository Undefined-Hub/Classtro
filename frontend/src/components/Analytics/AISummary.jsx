...
const AISummary = () => {
  ...
  const buildFallbackInsights = () => {
    const duration = sessionInfo.startAt && sessionInfo.endAt
      ? Math.max(1, Math.round((new Date(sessionInfo.endAt) - new Date(sessionInfo.startAt)) / (1000 * 60)))
      : 0;
    ...
    return {
      ...
      risks: [
        ...
        (sessionInfo.totalParticipants || 0) > (sessionInfo.peakParticipants || 0)
      ...
      scoreBreakdown: {
        ...
        engagement: Math.max(1, Math.min(100, Math.round(responseRate * 0.7 + (sessionInfo.totalParticipants || 0) * 0.8))),
        participation: Math.max(1, Math.min(100, Math.round((sessionInfo.peakParticipants || 0) * 2))),
        clarity: Math.max(1, Math.min(100, Math.round(questions.length ? (answeredQuestions / questions.length) * 100 : 0))),
      },
    };
  };
  ...
  const resolvedSummary = {
    ...
    scoreBreakdown:
      aiSummary.scoreBreakdown && Object.keys(aiSummary.scoreBreakdown).length > 0
  ...
  const renderSectionIcon = (key, colorClass) => {
    if (sectionIcon[key] === "bar") {
      return (
        <svg className={`w-4 h-4 ${colorClass}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6m6 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0h6m6 0V5a2 2 0 00-2-2h-2a2 2 0 00-2 2v14" />
        </svg>
      );
    }
    ...
    if (sectionIcon[key] === "users") {
      return (
        <svg className={`w-4 h-4 ${colorClass}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20h10M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
      );
    }
    ...
    if (sectionIcon[key] === "spark") {
      return (
        <svg className={`w-4 h-4 ${colorClass}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3l2.2 4.5L19 9l-3.5 3.3.8 4.7L12 14.8 7.7 17l.8-4.7L5 9l4.8-1.5L12 3z" />
        </svg>
      );
    }
    ...
    if (sectionIcon[key] === "alert") {
      return (
        <svg className={`w-4 h-4 ${colorClass}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v4m0 4h.01M10.29 3.86l-8 14A1 1 0 003.16 19h17.68a1 1 0 00.87-1.5l-8-14a1 1 0 00-1.74 0z" />
        </svg>
      );
    }
  ...
  const renderCompactSection = (section) => {
    if (section.type === "text") {
      return (
        <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed" style={clampStyle}>
          {section.value}
        </p>
      );
    }
      ...
      <ul className="space-y-1.5">
        {section.value.slice(0, 3).map((item, index) => (
          <li key={index} className="flex items-start gap-2">
            ...
            <span className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed" style={clampStyle}>
        ...
        <div className="fixed inset-0 z-[120] bg-black/50 backdrop-blur-sm p-4 sm:p-6 lg:p-10">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 max-w-4xl mx-auto h-full flex flex-col">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">AI Insights - Full View</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Generated {resolvedSummary.generatedAt ? new Date(resolvedSummary.generatedAt).toLocaleString() : "now"}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="p-5 overflow-y-auto space-y-5 hide-scrollbar">
              {visibleSections.map((section) => (
                <section key={`full-${section.key}`}>
                  ...
                  {section.type === "text" && (
                    <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{section.value}</p>
                  )}
                      ...
                      {section.value.map((action, index) => (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2 mb-1">
                            ...
                            <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${action.priority === "high" ? "bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300" : action.priority === "medium" ? "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300" : "bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300"}`}>
                              {action.priority}
                            </span>
                          </div>
                          <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">{action.why}</p>
                        </div>
                      ))}
                    </div>
                  )}
