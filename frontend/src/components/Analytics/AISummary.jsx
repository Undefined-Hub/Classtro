...
const AISummary = () => {
  ...
  const generateAISummary = () => {
    ...
    const avgPollResponses = Math.round(
      polls.reduce((sum, p) => sum + p.totalResponses, 0) / polls.length,
    );
    ...
    return {
      ...
      recommendations: [
        duration > 45
          ? "Consider breaking longer sessions into smaller segments for better retention"
          : "Session duration was optimal for student attention span",
        responseRate < 70
          ? "Encourage more Q&A interaction to improve response rates"
          : "Excellent Q&A engagement - maintain current interactive approach",
        polls.length < 3
          ? "Consider adding more polls to increase interactivity"
          : "Good use of polls for engagement",
          ...
          <ul className="space-y-2">
            {aiSummary.recommendations.map((recommendation, index) => (
              <li key={index} className="flex items-start">
                <svg
                  className="w-4 h-4 text-orange-500 dark:text-orange-400 mt-0.5 mr-2 flex-shrink-0"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path
