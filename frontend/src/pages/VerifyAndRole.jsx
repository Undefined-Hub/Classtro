...
export default function VerifyAndRole() {
    ...
    const { email, google, step: initialStep, oauth, accessToken, user } = location.state || {};
    ...
    const handleResendOTP = async () => {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) {
                setResendMessage(`New OTP sent successfully!${data.expiresIn ? ` (Expires in ${data.expiresIn} minutes)` : ""}`);
                setOtp("");
                safeToast.success(`New OTP sent (expires in ${data.expiresIn || 'N/A'} mins)`);
            } else {
